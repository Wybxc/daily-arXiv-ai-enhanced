...
TL;DR: GPT-5在低资源编程语言Idris中通过基于编译器错误的迭代提示显著提升性能，从22/56提升到54/56


<details>
  <summary>Details</summary>
Motivation: 研究GPT-5在低资源或不常用编程语言（如Idris）中的表现，探索如何通过反馈驱动的方法提升其在陌生语言环境下的编程能力

Method: 使用Exercism平台的Idris练习，首先建立零样本提示基线，然后评估多种精炼策略：基于平台反馈的迭代提示、增强文档和错误分类指南、使用本地编译错误和失败测试用例的迭代提示

Result: 零样本提示下GPT-5仅解决22/56个Idris问题，远低于Python（45/50）和Erlang（35/47）。使用本地编译错误的迭代提示效果最佳，将性能提升至54/56个问题

Conclusion: 大型语言模型在低资源环境中可能初始表现不佳，但结构化的编译器级别反馈在解锁其能力方面发挥关键作用
