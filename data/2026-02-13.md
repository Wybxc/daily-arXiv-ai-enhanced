...
TL;DR: GPT-5在Idris函数式编程语言中通过错误引导的迭代提示显著提升性能，从22/56提升到54/56，接近Python水平


<details>
  <summary>Details</summary>
Motivation: 研究GPT-5在低资源/不常用编程语言（如Idris）中的能力，探索如何通过结构化反馈机制提升其在陌生语言环境下的表现

Method: 使用Exercism平台的Idris练习进行评估，比较零样本提示与多种精炼策略：基于平台反馈的迭代提示、添加文档和错误分类指南、利用本地编译错误和失败测试用例的迭代提示

Result: 零样本提示仅解决22/56个问题；使用本地编译错误引导的迭代提示效果最佳，将性能提升至54/56个问题，接近Python的45/50水平

Conclusion: 大型语言模型在低资源环境中初始表现可能受限，但结构化编译器级反馈能显著解锁其能力，为模型在专业领域应用提供重要启示
