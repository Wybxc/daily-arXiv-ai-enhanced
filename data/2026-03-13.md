...
Motivation: 当前LLM在生成硬件描述语言（如Verilog）代码时面临高质量训练数据稀缺的挑战，现有方法主要关注语法正确性，导致生成的代码存在功能错误。

Method: 采用三阶段框架：1) LLM生成高层次Python参考模型定义电路行为；2) LLM生成初始Verilog RTL候选并迭代修复语法错误；3) 引入Concolic测试引擎执行深度时序逻辑并识别边界情况漏洞，提供周期精确执行轨迹和寄存器快照作为因果上下文。

Result: 在严格基准测试中实现超过80%的功能正确性，在VerilogEval-machine数据集上达到90.2%的pass@10分数；通过轨迹感知优化平均消除25%的冗余逻辑。

Conclusion: AutoVeriFix+通过整合高层次语义推理和状态空间探索，显著提升了LLM生成Verilog代码的功能正确性和设计效率，同时实现了有效的面积优化。
