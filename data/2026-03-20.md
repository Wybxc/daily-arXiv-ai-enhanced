...
TL;DR: 提出了一种基于特征集动态跟踪的JavaScript编译器优化方法，通过条件执行转译过程来减少不必要的AST遍历，显著提升编译性能


<details>
  <summary>Details</summary>
Motivation: 随着ECMAScript规范演进，工业级JavaScript编译器面临支持现代语法同时保持兼容性的挑战。传统编译器采用基于目标语言级别的固定转译流水线，导致大量计算浪费，因为编译器会执行昂贵的AST遍历来降低实际源代码中可能不存在的语言特性。

Method: 提出编译器改进方案：基于准确跟踪和动态维护编译单元中语言特性集合来条件执行转译过程。在Google Closure Compiler中实现，通过在JavaScript脚本级别填充和维护FeatureSet，动态跳过不必要的降低过程。设计了架构保障措施，包括策略性的过程排序和转译代码特征正确性的动态验证。

Result: 在大规模生产级monorepos上的评估显示，该改进显著减少了编译时间，节省了计算和内存使用。

Conclusion: 通过动态跟踪语言特性并条件执行转译过程，可以有效优化JavaScript编译器性能，减少不必要的计算开销，为工业级编译器提供了实用的性能改进方案。
...
TL;DR: TENSURE：首个专门针对稀疏张量编译器（STC）的可扩展黑盒模糊测试框架，使用爱因斯坦求和表示法生成复杂张量收缩运算，通过约束生成算法保证100%语义有效性，在TACO和Finch系统中发现大量崩溃和静默错误编译问题。


<details>
  <summary>Details</summary>
Motivation: 稀疏张量编译器（STC）在优化高维数据分析和机器学习工作负载中至关重要，但需要从高级声明式规范合成复杂、不规则的控制流，容易产生细微的正确性缺陷。现有测试框架基于标准运算符计算图变异，无法充分测试编译器的任意循环合成能力；通用基于语法的模糊测试器难以生成有效输入，因为索引在多张量间的重用规则严格。

Method: 1. 使用爱因斯坦求和（Einsum）表示法作为通用输入抽象，生成复杂、非常规的张量收缩运算以暴露STC代码生成阶段的边界情况；2. 提出新颖的基于约束的生成算法，保证合成内核100%语义有效性；3. 引入语义保持的变异算子，利用代数交换性和存储格式异构性实现无可信参考的蜕变测试。

Result: 在TACO和Finch两个最先进系统上的评估显示广泛脆弱性：TENSURE在TACO中暴露了大多数生成测试用例的崩溃或静默错误编译问题；约束生成算法显著优于基线语法模糊测试器约3.3%的有效率，实现100%语义有效性。

Conclusion: TENSURE作为首个专门针对稀疏张量编译器的可扩展黑盒模糊测试框架，揭示了STC生态系统中存在的广泛脆弱性，特别是TACO中的严重问题，强调了稀疏编译生态系统中需要专门测试工具的紧迫性。
...
TL;DR: LPG是一个利用大语言模型（LLM）来自动化孔优化（peephole optimization）泛化的框架，通过LLM的语义抽象和探索推理能力，结合形式化验证，显著提升了优化规则的泛化效果。


<details>
  <summary>Details</summary>
Motivation: 孔优化是现代优化编译器的核心组件，但创建新的优化规则需要从具体优化实例提升为更通用的重写规则。这一泛化步骤对优化效果至关重要，但通常需要大量人工努力和领域专业知识。现有自动化方法（如Hydra）存在搜索空间爆炸、泛化不足以及对不同指令域支持有限等问题。

Method: LPG采用闭环工作流程，集成LLM驱动的符号常量泛化、结构泛化、约束松弛以及位宽/精度泛化，并通过语法验证、语义验证和收益性检查提供反馈。该方法结合了LLM的语义抽象和探索推理能力与形式化分析确保规则的正确性和收益性。

Result: 在从LLVM生态系统中提取的真实孔优化问题上，LPG成功泛化了102个优化中的90个。在与Hydra直接可比的整数优化子集上，LPG泛化了81个中的74个，而Hydra仅泛化了35个。

Conclusion: LPG框架通过结合大语言模型的语义抽象能力和形式化验证，显著提升了孔优化规则的自动化泛化效果，在真实场景中表现优于现有方法，为解决编译器优化规则创建中的泛化难题提供了有效方案。
