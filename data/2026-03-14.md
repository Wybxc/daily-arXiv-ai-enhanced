...
Motivation: LLM在生成高级编程语言代码方面表现出色，但在硬件描述语言（如Verilog）方面面临挑战，主要原因是高质量训练数据稀缺。现有方法多关注语法正确性，导致生成的代码存在功能错误。

Method: 提出三阶段框架：1) LLM生成高层次Python参考模型定义电路行为；2) LLM生成初始Verilog RTL候选并迭代修复语法错误；3) 引入Concolic测试引擎执行深度时序逻辑并识别边界情况漏洞，利用周期精确执行轨迹和内部寄存器快照为LLM提供因果上下文，同时生成覆盖率报告识别功能冗余分支进行语义剪枝。

Result: 在严格基准测试中实现超过80%的功能正确性，在VerilogEval-machine数据集上达到90.2%的pass@10分数。通过轨迹感知优化平均消除25%的冗余逻辑。

Conclusion: AutoVeriFix+通过集成高层次语义推理和状态空间探索，显著提升了LLM生成Verilog代码的功能正确性和设计效率，解决了现有方法在硬件描述语言生成中的局限性。
